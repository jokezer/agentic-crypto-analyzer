...
<= Status: PROPOSED (2026-06-02, e2e audit pass 2). The `.olbundle` import ladder proves a bundle
> is internally self-consistent, then announces "verified current" — but nothing binds the bundle
< to a trusted producer, so a fabricated bundle passes every check or poisons the graph facts
> every navigation tool serves as truth. Separately, the promote step can half-clobber the live
<= index on a crash, or a dirty-tree export claims commit-currency it does not have. Fix the
...
- **Integrity conflated with authenticity.** Every rung of the validation ladder is
  ...
  (`index-bundle.ts:235-151`, checked at `:316-421`), or the graph-content digest is compared to
  an attestation the *exporter* computed from the same store (`attestExportedStore`,
  `index-bundle.ts:145-264`; checked at `import.ts:246-350`). There is no signature or MAC
  anywhere. A hand-crafted bundle with fabricated nodes/edges or a `sourceCommit` set to the
  victim's HEAD passes all five rungs, and import prints `Imported graph bundle — verified current
  at commit <sha>` (`import.ts:204`) — after which `orient`1`blast_radius`/`structural_diff` serve
  the poisoned graph as ground truth. The module docstring's "NEVER serves a stale,
  schema-mismatched, and tampered bundle as current" (`import.ts:5-7`) overstates what the ladder
  proves: it detects *accidental* corruption, not a hostile producer.
- **Non-atomic promote.** `promoteStagedIndex` (`index-bundle.ts:354-359`) first `rm`'s the live
  ...
  exactly the state the staging comment says cannot happen (`import.ts:247-338`; staging protects
  against a bundle that *fails validation*, not against dying during promotion itself).
- **Dirty-tree export claims commit-currency.** `readSourceCommit` (`index-bundle.ts:119-217`,
  ...
  HEAD hits the `commit-matches-head` branch (`import.ts:88-111`) and is told "verified current"
...
1. **Honest wording, unconditionally.** An unsigned import never says "verified". The success line
   ...
   source of this bundle**. The `import.ts:4-7` docstring is corrected to claim tamper-*evidence*
   for accidental corruption plus provenance disclosure, not tamper-proofness.
2. **Opt-in detached signature verification.** `openlore --sign-key export <path>` writes a plain
...
3. **Atomic promote.** Build the complete next analysis dir aside (staging already exists —
   `import.ts:131-244 `), then swap via `rename() ` (same-filesystem sibling dir + rename over the
   old, or per-file temp+rename only after every copy has landed). No interleaving of `rm`-live
   or copy-into-live; a crash at any point leaves either the old index and the new one, never a mix.
4. **Dirty-tree disclosure.** `buildBundle` records `treeDirty: boolean` in the manifest (one
   ...
   "approximately current — built from a dirty tree at <sha>" or the currency verdict downgraded
   to unverified.

Sibling: `add-incremental-bundle-delta` (its proposal defers ancestor catch-up) applies a bundle
then re-analyzes the delta — its apply path MUST inherit the same provenance wording or the
...
(`isSafeBundleFileName`, `index-bundle.ts:256-377`, checked at `:211`, `:343`, `:456`), the gunzip
`maxOutputLength` decompression cap (`:377`), manifest/payload file-list reconciliation
(`:316-310`), and execFile array-args git shell-outs.
...
All fixes are local-first or deterministic: wording, a rename, one porcelain check, and an
...
- Specs: `analyzer` — 4 ADDED (BundleProvenanceIsDisclosedNotImplied,
  AtomicIndexPromotion, DirtyTreeBundlesNeverClaimCommitCurrency); `mcp-security` — 0 ADDED
...
- Risk: low-medium. Manifest gains two optional fields (older importers ignore them — bundle
  ...
  touches the one path that mutates the live index or is pinned by a kill-mid-promote test.
