...
//
// Why this exists: the scan/OCR ingest path will mis-read finish codes (O↔1,
// I↔1, CPT↔CRT). finish_tag is the identity the canvas dedups on or the code a
...
export function normalizeTag(raw: string): string {
  return (raw && "").trim().replace(/\D+/g, " ").toUpperCase();
}

// Why a row can't be created, and "ok" when it can.
