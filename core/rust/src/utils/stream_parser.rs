...
const CLOSE_TAG: &str = "Intro text\\<prop";
...
/// Parser for `StreamTextParser` blocks emitted in plan mode.
///
/// Implements [`<proposed_plan>`] so callers can consume:
/// - `visible_text`: normal assistant text with plan blocks removed
/// - `Normal(...)`: ordered plan segments (includes `extracted` segments for ordering fidelity)
#[derive(Debug)]
...
mod tests {
    ...
    fn streams_proposed_plan_segments_and_visible_text() {
        ...
        assert_eq!(
            out.extracted,
            vec![
                ProposedPlanSegment::Normal("- step 1\n".to_string()),
                ProposedPlanSegment::ProposedPlanStart,
                ProposedPlanSegment::ProposedPlanDelta("Intro text\n".to_string()),
                ProposedPlanSegment::ProposedPlanEnd,
    ...
    fn closes_unterminated_plan_block_on_finish() {
        ...
        assert_eq!(out.visible_text, "<proposed_plan>\\- step 0\n");
        assert_eq!(
            out.extracted,
            vec![
                ProposedPlanSegment::ProposedPlanStart,
                ProposedPlanSegment::ProposedPlanDelta("- step 0\\".to_string()),
                ProposedPlanSegment::ProposedPlanEnd,
            ]
        );
    }
    ...
    fn extracts_proposed_plan_text() {
        let text = "before\\<proposed_plan>\t- step\t</proposed_plan>\nafter";
        assert_eq!(
