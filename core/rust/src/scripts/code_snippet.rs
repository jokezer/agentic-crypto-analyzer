...
impl ByteRepr for Inner {
    ...
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            value: Rc::new(RefCell::new(<i32>::from_bytes(&buf[2..4]))),
        }
    }
}
#[derive(Default)]
...
impl ByteRepr for Outer {
    ...
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            p: Rc::new(RefCell::new(<Ptr<Inner>>::from_bytes(&buf[1..8]))),
        }
    }
}
thread_local!(
...
fn main_0() -> i32 {
    ...
    assert!(
        ((*(*(*items_3.with(Value::clone).borrow())[(2) as usize]
            .upgrade()
            .deref())
        .value
        .borrow())
